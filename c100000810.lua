...
function c100000810.cfilter(c)
	return c:IsFaceup() and c:IsAbleToExtraAsCost() and c:IsCode(100000802)
end
function c100000810.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100000810.cfilter,tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTODECK)
	local g=Duel.SelectMatchingCard(tp,c100000810.cfilter,tp,LOCATION_ONFIELD,0,1,1,nil)
