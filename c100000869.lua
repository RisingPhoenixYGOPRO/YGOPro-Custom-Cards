...
function c100000869.filterrs(c,e,tp)
	return c:IsFaceup() and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsSetCard(0x10E) and c:IsType(TYPE_MONSTER)
end
...
function c100000869.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x10E) and c:IsType(TYPE_MONSTER)
end
...
local c=e:GetHandler()
	return bit.band(r,REASON_EFFECT) and re:GetHandler():IsSetCard(0x10E)
end
...
function c100000869.filter(c)
	return c:IsSetCard(0x10E) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not c:IsCode(100000869)
end
