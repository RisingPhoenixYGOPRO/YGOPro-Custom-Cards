...
function c388.op(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:GetFlagEffect(388)==0 then
			...
			e5:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_SET_AVAILABLE+EFFECT_FLAG_UNCOPYABLE)
			tc:RegisterEffect(e5)
			local e6=Effect.CreateEffect(tc)
			e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
			e6:SetCode(EFFECT_CHANGE_RANK)
			e6:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_SET_AVAILABLE+EFFECT_FLAG_UNCOPYABLE)
			e6:SetValue(0)
			tc:RegisterEffect(e6)
			tc:RegisterFlagEffect(388,0,0,1) 
		end
		tc=g:GetNext()
	end
end
...
function c388.addc(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():AddCounter(0x1088,e:GetHandler():GetOriginalRank())
end
